...
def convert(low, high):
	data = bus.readU8(low) | bus.readU8(high) << 8;
	if (data > 32767) :
		data -= 65536;
	return data;
...
# bus.write_byte_data(acclAddr, CTRL_REG1_XM, 0x0F);	# 0x67		Accel data rate 	= 100Hz, 		Power ON
# bus.write_byte_data(acclAddr, CTRL_REG1_XM, 0x1F);	# 0x67		Accel data rate 	= 100Hz, 		Power ON
# bus.write_byte_data(acclAddr, CTRL_REG1_XM, 0x2F);	# 0x67		Accel data rate 	= 100Hz, 		Power ON
bus.write_byte_data(acclAddr, CTRL_REG1_XM, 0x3F);	# 0x67		Accel data rate 	= 100Hz, 		Power ON
# bus.write_byte_data(acclAddr, CTRL_REG1_XM, 0x4F);	# 0x67		Accel data rate 	= 100Hz, 		Power ON
# bus.write_byte_data(acclAddr, CTRL_REG1_XM, 0x5F);	# 0x67		Accel data rate 	= 100Hz, 		Power ON
# bus.write_byte_data(acclAddr, CTRL_REG1_XM, 0x6F);	# 0x67		Accel data rate 	= 100Hz, 		Power ON
# bus.write_byte_data(acclAddr, CTRL_REG1_XM, 0x7F);	# 0x67		Accel data rate 	= 100Hz, 		Power ON
# bus.write_byte_data(acclAddr, CTRL_REG1_XM, 0x8F);	# 0x67		Accel data rate 	= 100Hz, 		Power ON
# bus.write_byte_data(acclAddr, CTRL_REG1_XM, 0x9F);	# 0x67		Accel data rate 	= 100Hz, 		Power ON
# bus.write_byte_data(acclAddr, CTRL_REG1_XM, 0xAF);	# 0x67		Accel data rate 	= 100Hz, 		Power ON
...
try:
	...
	while True:
		...
		write(" | ", convert(bus.read_byte_data(acclAddr, OUT_X_L_A), bus.read_byte_data(acclAddr, OUT_X_H_A)));
		write(" | ", convert(bus.read_byte_data(acclAddr, OUT_Y_L_A), bus.read_byte_data(acclAddr, OUT_Y_H_A)));
		write(" | ", convert(bus.read_byte_data(acclAddr, OUT_Z_L_A), bus.read_byte_data(acclAddr, OUT_Z_H_A)));
